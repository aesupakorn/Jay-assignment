...
for i in stations:
    ...
    alllong.append([stations[i]['long'],i])
alltemp.sort()
alllat.sort()
alllong.sort()
...
def peak_stations(stations):
    peak = []
    peak.append(alllat[0][1])
    peak.append(alllat[len(alllat)-1][1])
    peak.append(alllong[0][1])
    peak.append(alllong[len(alllong)-1][1])
    return peak
